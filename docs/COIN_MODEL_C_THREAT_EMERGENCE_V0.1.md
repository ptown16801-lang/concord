...
**Normative input:** [Coin model A v0.1](https://github.com/ptown16801-lang/concord/blob/652f3f7b13214bf7e651b0228f654621f568a641/docs/COIN_MODEL_A_V0.1.md), the original specification input (superseded for current conformance by revision 7073e9a and ECON-02 below)
...
| --- | --- | --- |
| `N0 no activation` | After exposure, no agent initiates a coin primitive or coin-related communication | Entry is the initial condition; exit occurs on the first use. Remaining in `N0` is valid data when exposure and capture are adequate; otherwise observation is censored |
...
| `N3 secondary circulation` | Any actual post-issuance transfer of a legitimate coin or appearance occurs; report first transfer, third-party transfer, issuer reacquisition and retransfer separately | One transfer is sufficient for event classification, not for declaring a stable regime |
...
| `N7 issuer dilution` | An increase in a named supply/exposure view (section 7.2) for one issuer covaries with reduced acceptance, offer value, retention, or use of that issuer's coins | Unlimited issuance is still permitted; covariance is not automatically causal |
...
| H3 | Secondary circulation occurs independently of direct issuer involvement | First-secondary-transfer fraction among issued identities/appearances with observation opportunity; all post-issuance transfers, issuer reacquisition/retransfer, third-party reach and depth separately | Copies, false transfers, repeated pair transfers, unequal opportunity |
...
| H7 | Increased issuer supply is associated with issuer dilution in some contexts | Changes in named legitimate-identity, appearance or perceived-supply views followed by separately evidenced response/offer/retention/transfer outcomes; association only absent a causal design | Recipient/context composition, copied supply, issuer behavior changes, time trends |
...
| H11 | Refusal is selective rather than uniform for some agents | Within-agent response-category rates under section 7.2 by issuer, counterparty, condition, marks, quantity and context on identifiable comparable proposals | Sparse opportunities, bundles, communication failures, missing refusal/acceptance evidence |
...
### 7.2 JON-105 measurement disposition — September 24

This section implements all six requirements from [JON-105 handoff](https://linear.app/jons-garage/issue/JON-99#comment-e7cdf8aa-7c72-4e4a-9e39-a2e6393e473a). They are research definitions, not additional coin mechanics.

| Requirement | Operational definition and reporting rule |
| --- | --- |
| 1. Disaggregate non-use | Report run no-activation (N0), actor non-issuance, non-engagement after documented exposure/opportunity, proposal non-uptake, appearance dormancy, issuer discontinuation and explicit displacement separately. Missing/censored observation is a separate quality state, never evidence of refusal or indifference. |
| 2. Response evidence | For each identifiable proposal/revision, retain explicit acceptance, behavioral uptake, explicit refusal, counteroffer/conditional response, return, destruction and no observed response as distinct evidence/event categories with timestamps. They can co-occur in sequence; specify the window and endpoint rule rather than forcing exclusive labels. Inferred response is a separate analysis with uncertainty. Receipt, retention and silence are not acceptance. A rate names its response numerator, eligible proposal-opportunity denominator, audience and capture coverage; unknown eligibility is excluded and counted as missing. |
| 3. Secondary transfer | Any actual post-issuance transfer is secondary, including return to the issuer. Report first-secondary incidence, third-party transfers whose transferor is not the original issuer, issuer reacquisition/retransfer, transfer depth and unique actor reach separately at legitimate-identity and appearance levels. H3's first-secondary fraction is not an estimate of all circulation. A return does not prove refusal; a transfer does not prove a sale. |
| 4. Named supply views | Distinguish cumulative legitimate coin identities ever issued; extant legitimate identities with at least one observed live appearance; appearance stock split original/copy/fabrication; and audience-specific perceived supply reconstructed from evidence available to that audience. Unknown lifecycle and capture gaps remain explicit; do not silently subtract destroyed appearances from cumulative issuance or treat fabrication as legitimate issuance. |
| 5. Dilution exposures | Legitimate issuance, copy/fabrication proliferation, perceived-supply exposure and feature/signaling saturation are distinct predictors. Preserve which agent could observe which exposure when. Endogenous ordinary-run issuance supports association, not a causal dilution effect. A causal claim requires an independently justified design with assignment/confounding assumptions, controls and uncertainty; none is established here. |
| 6. Units and uncertainty | Declare action, allocation, coin, instance, actor, issuer, dyad and run units as applicable. Link batch child events and retries so an action is not counted repeatedly; do not treat dependent transfers as independent samples. Report numerators/denominators, observation windows, right-censoring, missingness and clustered uncertainty at the dependence/assignment unit. No single emergence score or preferred economic outcome replaces these reports. |

For mixed-resource bundles retain all terms and context; an observed bundle is not a marginal coin price. Event-time visible information, later hidden facts, agent statements and analyst inference remain separate. Each of the six rows is a required analysis-report acceptance check, not an experiment already run. The existing JON-105 report remains the independent source; it is not replaced by this producer disposition.
