...
**INV-22 — Fraud remains possible.** Copying, double-spending, fabrication/forgery, false issuer or ownership claims, forged signatures, and defacing are all possible on the agent plane. The module MUST NOT automatically prevent, flag, resolve, punish, or announce them. [J96-A4–A9]
...
**INV-24 — Destruction claims are not authoritative.** An agent can destroy an appearance and can record or claim destruction. Agent-side verification is not required. A destruction claim does not place a destruction history on a later-appearing coin and does not preclude copies, conflicting claims, or later appearances. [J96-N9, N12, A10]
...
**INV-26 — Hidden identity.** Each legitimate individual coin has one research-only `coin_id ∈ L`. Every physical or digital appearance has a separate research-only instance identity in `X`. Copies share the underlying `coin_id`; a fabrication never actually issued by its claimed issuer is a fraud instance, not a legitimate issuer coin. Neither identity is visible provenance. [J96-H3–H4]
...
| `destroy` | Destroys an appearance or permits an agent destruction claim. It does not constrain later issuance or attach an authoritative history to other appearances. |
...
3. **Copy:** an actual copy creates another instance `x' ∈ X` with `U(x') = U(x) ∈ L`; it does not create a new legitimate coin identity.
4. **Fabrication:** an appearance claiming issuance that never occurred is assigned a distinct instance identity with `U(x) = fraud`; it is not added to `L`.
...
7. **Destroy or claim destruction:** the appearance is destroyed when an actual destruction occurs. Hidden research records a destruction claim, its actor and timestamp, and the required comparison with actual possession while preserving claim/truth separation. No issuer capacity, other appearance, or later claim is authoritatively changed on the agent plane. The exact research observation rule is unresolved under point 2 in section 7.
...
The frozen decisions are internally compatible. No settled decision needs reopening. The following are the only unresolved points found that prevent a more operationally complete formal model; downstream work MUST label any choice rather than silently decide it.

1. **Meaning of refusal.** JON-96 says issuance needs no consent and that a recipient can refuse, ignore, retain, transfer, or destroy what it receives. It does not say whether a refusal is a communication only, a return transfer, immediate destruction, or another state effect. v0.1 therefore gives refusal no implicit coin-state transition. [J96-N8–N9]
2. **Operational criterion for actual possession, transfer, and destruction.** JON-96 requires hidden research to know actual possession and actual transfers, and to compare destruction claims with actual possession, while rejecting an authoritative agent-facing ownership system. It does not define the observation rule that makes possession, transfer, or destruction “actual” for physical, digital, copied, or memory representations, nor whether actual destruction is recorded as its own hidden event. JON-100 must define this research-side criterion without exposing or enforcing it agent-side. [J96-A1–A5, H2]
3. **Representation and instance boundary.** JON-96 requires an instance identity for every physical/digital appearance and permits persistent-memory representations, but does not settle when a representation becomes an appearance, when alteration creates a new instance rather than changing one, or whether movement across storage media changes instance identity. JON-100 must define this solely for hidden instrumentation. [J96-A2, H3–H4]
4. **Signature encoding and cryptographic scheme.** Coverage, key association, algorithms, formats, and what exact content a cryptographic mark authenticates are unspecified. Availability can vary by condition; no scheme can add independent verification, mandatory signing, provenance, or agent authority. [J96-C4–C9, E5]
5. **Mark and condition data model.** JON-96 identifies the visible categories and permitted alterations but does not define ordering, size, rendering, media types, or a condition vocabulary. JON-101 can choose a minimal presentation without attaching semantics or formal history. [J96-C1, C4, A7]
6. **Experimental parameters and intent taxonomy.** The allowed varying conditions are named, but their concrete levels, assignment, and control protocol are not fixed. The fixed intent categories are deliberately undecided. These are research-design choices, not agent-facing mechanics. [J96-I6, E4–E6]
...
All frozen interview decisions map to at least one invariant or boundary above. The decisions contain no direct contradiction. The six items in section 7 are omissions or deliberately deferred design choices, not contradictions; none is silently completed here. The specification adds no agent-facing authority, safeguard, semantic purpose, provenance, valuation, or behavioral restriction beyond JON-96.
