...
function extractPosition(contact, event, viewport) {
  ...
  const width = finite(viewport?.width ?? event.viewport?.width);
  const height = finite(viewport?.height ?? event.viewport?.height);
...
function eventTimestamp(event, declaredStartedAt) {
  if (declaredStartedAt != null && Number.isFinite(Number(event?.elapsedMs))) {
    return declaredStartedAt + Number(event.elapsedMs);
  }
