...
export async function simulateScenario({ scenario, proposeAssignments, checkAssignments }) {
  ...
  for (let round = 0; round < source.roundCount; round++) {
    ...
    let proposals = proposeAssignments ? await proposeAssignments(snapshot) : deterministicBaseline({ agents: state.agents, tasks: active, round, history });
    proposals = proposals ?? {};
    ...
    for (const task of active) {
      ...
      let agentId = checker.outcome === "approved" ? proposedAgentId : checker.outcome === "failed" ? baseline[task.id] : null;
      ...
      else task.incumbent = null;
      records.push({ taskId: task.id, proposedAgentId, checker: { outcome: checker.outcome, reason: checker.reason ?? null }, appliedAgentId: agentId, source: checker.outcome === "failed" ? "deterministic-fallback" : checker.outcome === "approved" ? "candidate" : "none", continuation, handoff, completed: task.remaining === 0, remaining: task.remaining, gap: agentId === null ? (checker.outcome === "rejected" ? "checker-rejected" : "capacity-or-eligibility") : null, waitRounds: agentId === null ? round - task.arrivalRound + 1 : 0, deadlineRound: task.deadlineRound ?? null, urgency: task.urgency ?? null });
    }
