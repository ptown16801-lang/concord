...
class ConcordFingerElement extends HTMLElement {
  ...
  _closeRequested() {
    if (!this._active || this._scanComplete || this._closing) return;
    this._closing = true;
  ...
  _finish() {
    ...
    this._data.endedAt = new Date().toISOString();
  ...
  async _submit(data) {
    const completion = data.lifecycle.findLast?.((entry) => entry.type === 'scan-complete');
    const payload = {
      ...
      termination: completion?.reason ?? 'continue',
      viewport: data.events.find((event) => event?.viewport)?.viewport ?? null,
      device: { ...data.capability, observedMaxContacts: data.observedMaxContacts },
      events: data.events,
      dom: { initial: data.initialDom, final: data.finalDom, mutations: data.mutations },
      lifecycle: data.lifecycle,
    };
    try {
      await fetch(this._config.ingestUrl, {
        method: 'POST',
