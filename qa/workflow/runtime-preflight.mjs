...
export function assertCompatibleNode(version = process.versions.node) {
  ...
  if (!compatible) {
    throw new Error(
      `Node.js 22.5.0 or newer is required; found ${version}. Use a compatible Node.js runtime and retry.`,
    );
  }
}
...
export async function collectPreflightReport() {
  ...
  try {
    ...
    throw new Error("The in-memory node:sqlite compatibility check failed.");
  } finally {
    database?.close();
  }
